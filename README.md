...
_Judged with (mixed — fallback used):_ `nex-agi/nex-n2.5-pro:free` (1,357), `google/gemma-4-26b-a4b-it:free` (15), `google/gemma-4-31b-it:free` (15)

| Metric | This week | All-time |
|--------|----------:|---------:|
| Papers scanned | 592 | 9,827 |
| Papers with language mentions | 274 | 4,317 |
| Unique languages detected | 124 | 536 |
| Weeks tracked | — | 19 (since 2026-04-27) |

_9 paper(s) across 2 earlier week(s) are still awaiting an LLM judge verdict — backfilling in the background, with each catch-up run improving those weeks' "studied" counts._
...
```bash
make test       # full suite: pytest (Python) + vitest (web frontend)
make test-py    # Python only
make test-web   # web frontend only
```
