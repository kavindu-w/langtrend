...
├── run_logged.sh               Wraps a make target: tee output to logs/, then notify
└── notify.sh                   Desktop/webhook notification on a wrapped command's completion
```
...
| `judge_languages.py` | LLM-as-judge verification: for each paper in `*_detected.jsonl`, assembles context from the cached section text and asks an OpenAI-compatible model (whichever provider/model `LLM_JUDGE_BASE_URL`/`LLM_JUDGE_MODEL` point at — Cerebras, Groq, and OpenRouter free tiers, and local Ollama, all work; see `.env.example`) whether each detected language is `studied`, `mentioned_only`, or a `false_positive`. Writes per-paper verdicts to `judge_cache/`; resumable (cached papers are skipped). `--sweep-all-weeks` judges pending papers across every committed week (newest first) rather than one date range — used by the daily catch-up workflow to backfill history in the background. `--check-only` alone (no `--sweep-all-weeks`) just reports whether the single current/latest week is fully judged, without judging anything — this is what gates the CI deploy, so a historical backlog never blocks publishing this week's data. Single-paper testing: `--paper-id <id> [--dry-run]`. | `make judge` |
...
| `run_logged.sh` / `notify.sh` | Every `make` target is wrapped in `run_logged.sh`, which tees output to `logs/<target>_<timestamp>.log` and calls `notify.sh` with the exit code — fires a macOS notification or webhook (`NOTIFY_METHOD`/`NOTIFY_WEBHOOK_URL` in `.env`) so long-running local runs (a full week of `judge` or `process`) don't need to be watched. | called automatically by every `make` target |
