...
| `llm_client.py` | Thin client for any OpenAI-compatible `/chat/completions` endpoint — provider/model are whatever `LLM_JUDGE_BASE_URL`/`LLM_JUDGE_MODEL` point at (Cerebras, Groq, OpenRouter, Ollama, Gemini all work; see the root README and `.env.example`). `LLM_JUDGE_FALLBACK_MODELS` chains additional models on the same endpoint, each retried up to 3× before falling over. Distinguishes daily-quota exhaustion (`QuotaExhaustedError`, stop cleanly) from transient per-minute throttling (retry with backoff) and from a rejected key (`AuthenticationError`, account-wide so it skips the rest of the fallback chain), and paces requests to `LLM_JUDGE_RPM`/`LLM_JUDGE_RPH`. `ping()` also warns (never fails) when a configured model is absent from the provider's `/models` catalog. |
